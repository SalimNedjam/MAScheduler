...
// dimension of mat[][]
void getCofactor_more(int mat[N+2][N+2], int temp[N+2][N+2], int p, int q, int n)
{
	int i = 0, j = 0;
...
/* Recursive function for finding determinant of matrix.
   n is current dimension of mat[][]. */
int determinantOfMatrix_more(int mat[N+2][N+2], int n)
{
	int D = 0; // Initialize result
	...
	for (int f = 0; f < n; f++) {10
		// Getting Cofactor of mat[0][f]
		getCofactor_more(mat, temp, 0, f, n);
		D += sign * mat[0][f] * determinantOfMatrix_more(temp, n - 1);
...
int job_more()
{
	...
	return determinantOfMatrix_more(mat, N+2);
}
